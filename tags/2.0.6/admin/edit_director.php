...
if ($HTTP_POST_VARS['id_direct']) {
  $var=$HTTP_POST_VARS;
  
  if ($var['year']=='') $var['year']=0;
  if ($var['country']=='') $var['country']='null';

  $result=mysql_query("UPDATE ".$db_prefix."director SET name='".$var['name']."', country=".$var['country'].", year=".$var['year']." WHERE id=".$var['id'].";");
  ...
  $result=mysql_query("SELECT name,country,year FROM ".$db_prefix."director WHERE id='".$HTTP_POST_VARS['id']."';");
  $arr = mysql_fetch_array ($result);
  mysql_free_result($result);
  print "<html><body>\n";
  print "<center><h1>".$e_director."</h1><form action=\"edit_director.php\" method=\"post\">\n";
  print "<input type=\"hidden\" name=\"id_direct\" value=1>\n";
